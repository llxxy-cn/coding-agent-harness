...
- Task 18 is only complete for the current release's local package-build, container-verification, and CI-configuration subset. Task 19 is only complete for agent-owned documentation and license work; `REFLECTION.md` remains student-authored and pending. The completed historical `v0.2.0` Tag, hosted Release, and assets are not the current release candidate. The current `0.2.1` candidate's Tag, hosted GitHub Release, and release assets remain pending until the current PR is merged.
...
| 18 | `99b615e`, `8b66fe2`, `105b9c9` | PARTIAL | At this historical checkpoint, package build and CI definitions passed while `Dockerfile` and `.dockerignore` had not yet been created; later Docker delivery evidence is recorded below. |
...
| 20 | `3e7b312`; Render deployment source `task/final-course-delivery @ 9db2c1d40b3f117165283bf16423b51b26b38df78` | PARTIAL | The historical `v0.2.0` Tag, Release, and assets are complete; the current `0.2.1` candidate's Tag, hosted GitHub Release, and release assets remain pending until the current PR is merged. Local Linux Docker build and restricted real three-scenario smoke are complete; OCI Registry publication, remote GitLab pipeline execution, release manifest, and `verify_release.py` remain unverified or incomplete. The later Render evidence records `https://coding-agent-harness-ch7b.onrender.com`, `/healthz` HTTP 200, the new three scenario cards on the home page, and all three fixed-scenario terminal states. |
...
| Original Task 17 public-WebUI scope | `8aae4f6f962d877a9860de7412589fc55ff92e19`; Render deployment source `task/final-course-delivery @ 9db2c1d40b3f117165283bf16423b51b26b38df78` | **IMPLEMENTED (course demonstration)** | The original public demo/deployment plan was superseded by the fixed offline v0.2.0 WebUI plan. The public Render endpoint `https://coding-agent-harness-ch7b.onrender.com` is a fixed offline-scenario demonstration, not a free-form conversational Agent UI or highly available production service; `/healthz` returned HTTP 200, the home page displayed the new three scenario cards, and the fixed scenarios reached their expected terminal states: `feedback_success → succeeded`, `governance_denied → stopped`, and `human_review_pause → paused_for_human`. Each result page displayed the new status badge and typed, redacted execution trace. Render Free may sleep after idle time, so its first request can take more than 50 seconds. No OCI image publication or remote GitLab result is recorded. |
| v0.2.0 fixed offline WebUI | `e828b153`, `dd43bba`, `fcdca69`, `0f18c58`, `625a1b1`, `30e45b5`, `1035782`, `370ac49`, `47aa9e9`, `0f6945a`, `34db102` | IMPLEMENTED LOCALLY | Typed traces, fixed scenarios, application security, pages, supervised run service, packaged resources, CLI entry point, and the browser-Origin fix were merged through PRs #6–#16. The annotated tag points at PR #16 merge `34db102`. This row records the offline implementation evidence; the later Render row records the public deployment evidence. |
...
| Task 18 Docker/CI contract | `99b615e`, `0f6945a`; current CI repair uncommitted | PARTIAL | The GitLab `unit-test`, `package-build`, and `docker-build` jobs are configured; the GitLab configuration has local contract-test evidence, while no remote GitLab pipeline result is recorded. `Dockerfile` and `.dockerignore` exist, and a local Linux Docker daemon completed the image build plus restricted real three-scenario smoke (read-only rootfs, non-root, tmpfs, cap-drop, and no-new-privileges). No OCI image Registry publication is recorded. |
...
| Task 20 publication/deployment | `3e7b312`, `34db102`; Render deployment source `task/final-course-delivery @ 9db2c1d40b3f117165283bf16423b51b26b38df78` | PARTIAL | `v0.2.0` is a completed historical annotated tag and Release; the current source/package version is `0.2.1`, whose Tag, hosted GitHub Release, and release assets remain pending until the current PR is merged. The public Render endpoint `https://coding-agent-harness-ch7b.onrender.com` is a fixed offline-scenario demonstration, not a free-form conversational Agent UI or highly available production service; `/healthz` returned HTTP 200, the home page displayed the new three scenario cards, and the fixed scenarios reached their expected terminal states: `feedback_success → succeeded`, `governance_denied → stopped`, and `human_review_pause → paused_for_human`. Each result page displayed the new status badge and typed, redacted execution trace. A local Linux Docker daemon completed the restricted real three-scenario image smoke; no OCI image publication or remote GitLab pipeline result is recorded. |

**Historical version-consistency checkpoint:** at the `v0.2.0` historical release checkpoint, its WebUI tag named a release state while package metadata still declared `0.1.0`. The current project/package metadata is `0.2.1`; this correction does not move, delete, or rebuild the completed historical `v0.2.0` Tag or Release.

### v0.2.1 Package-metadata Consistency Release Candidate — 2026-08-11

`0.2.1` is the current release candidate for correcting the package-metadata inconsistency of the completed historical `v0.2.0` release. Its only current-version authorities are `[project].version` in `pyproject.toml` and `coding_agent_harness.__version__`; both are `0.2.1`. The `v0.2.1` Tag, hosted GitHub Release, and release assets remain pending until the current PR is merged. Historical v0.1.0 release notes, release URL, artifact names, and the completed annotated `v0.2.0` tag remain historical evidence and are not rewritten.
...
**Verification boundary:** the GitLab `docker-build` job is configured and locally contract-tested, but its remote pipeline execution is not recorded. A local Linux Docker daemon successfully built and ran the image with read-only rootfs, tmpfs `/tmp`, non-root execution, dropped capabilities, and `no-new-privileges`, completing the real three-scenario smoke. The image is not published to an OCI Registry. This stage remains single-instance because the demo run lock is process-local. The Stage 3A checkpoint and its local verification do not themselves prove a cloud deployment, Registry publication, remote GitLab CI result, Tag, Release, commit, or PR; later Render deployment evidence is recorded in the status table above.
...
- External registry publication and additional deployment work beyond the recorded Render `/healthz` verification.
